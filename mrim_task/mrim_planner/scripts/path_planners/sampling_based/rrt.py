...
class RRT:
    ...
    def getRandomPointGaussian(self, sigma_offset=0.0):
        ...
        while not point_valid:
            ...
            x = np.random.normal(mean[0], sigma[0], 1)[0]
            y = np.random.normal(mean[1], sigma[1], 1)[0]
            z = np.random.normal(mean[2], sigma[2], 1)[0]
    ...
    def getParentWithOptimalCost(self, point, closest_point, neighborhood):
        ...
        for neighbor in neighborhood_points:
            ...
            # Calculate the cost of the path if this neighbor were the parent
            potential_cost = self.tree.get_cost(neighbor) + distEuclidean(neighbor, point)
